...
if __name__ == '__main__':
    # For Device selection. Either GPU or CPU
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    parser = argparse.ArgumentParser()

    parser.add_argument('--input_channel', type=int, default=96, help='dim of input, the same as n_channel')
    ...
    parser.add_argument('--groups', default=1, type=int, help='set largest to 1 as ResNeXt')
    ...
    model = ResNet1D(in_channels= args.input_channel, base_filters= args.base_filter, kernel_size= args.kernel, stride= args.stride, groups=args.groups,
                     n_block= args.n_block, n_classes= args.n_classes, downsample_gap=2, increasefilter_gap=4, use_bn=True, use_do=True, verbose=True).to(device)
    x = torch.randn(32, 96, 72).to(device)
    y = model(x)
    print(y.shape)
